...
select puesto, nombre, apellido1, apellido2, email from empleado order by codigo_jefe limit 1 ;
...
select estado as 'estados de pedidos' from pedido group by estado;
...
select pago.codigo_cliente, fecha_pago from pago where  year(fecha_pago) = 2008;
-- o Utilizando la función DATE_FORMAT de MySQL.
select codigo_cliente, date_format(fecha_pago, '%d-%m-%Y') from pago where fecha_pago like ('2008%');
-- o Sin utilizar ninguna de las funciones anteriores.
select pago.codigo_cliente, fecha_pago from pago where fecha_pago like ('%2008%');
...
-- o Utilizando la función ADDDATE de MySQL. TIENE ERROR.
select codigo_pedido, codigo_cliente, fecha_esperada, fecha_entrega, estado from pedido where ( adddate(fecha_esperada, 2)) and (estado = 'entregado');
-- o Utilizando la función DATEDIFF de MySQL.
select codigo_pedido, codigo_cliente, fecha_esperada, fecha_entrega, estado from pedido where  (datediff(fecha_esperada, fecha_entrega) = 2) and (estado = 'entregado');
...
select * from cliente where (ciudad like 'Madrid') and (codigo_empleado_rep_ventas between 11 and 30);
-- Consultas multitabla (Composición interna)
-- Las consultas se deben resolver con INNER JOIN.
-- 1. Obtén un listado con el nombre de cada cliente y el nombre y apellido de su representante de ventas.
select nombre as 'nombre de Representante de ventas', apellido1, apellido2, codigo_empleado, nombre_cliente, codigo_empleado_rep_ventas from cliente inner join empleado on codigo_empleado = codigo_empleado_rep_ventas;
-- 2. Muestra el nombre de los clientes que hayan realizado pagos junto con el nombre de sus representantes de ventas.
SELECT c.nombre_cliente,e.nombre from cliente c inner join empleado e on c.codigo_empleado_rep_ventas = e.codigo_empleado inner join pago p where p.codigo_cliente = c.codigo_cliente;
-- 3. Muestra el nombre de los clientes que no hayan realizado pagos junto con el nombre de sus representantes de ventas.
select c.nombre_cliente,e.nombre,p.fecha_pago,p.id_transaccion from cliente c inner join empleado e on c.codigo_empleado_rep_ventas = e.codigo_empleado join pago p where p.codigo_cliente != c.codigo_cliente;
-- 4. Devuelve el nombre de los clientes que han hecho pagos y el nombre de sus representantes junto con la ciudad de la oficina a la que pertenece el representante.
SELECT c.nombre_cliente,e.nombre from cliente c inner join empleado e on c.codigo_empleado_rep_ventas = e.codigo_empleado inner join pago p where p.codigo_cliente = c.codigo_cliente;
-- 5. Devuelve el nombre de los clientes que no hayan hecho pagos y el nombre de sus representantes junto con la ciudad de la oficina a la que pertenece el representante.1
